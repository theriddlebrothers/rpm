...
class RB_Loader {
	...
	public function _load_path($_ci_path, $_ci_view) {
	...
	function _ci_load($_ci_data)
	{
		...
		// @edit
		// Check the /clients/ views folder for overriden view file
		if (IS_CLIENT) {
		
			// if view specifies the template, replace it wiht the client
			if (stripos($_ci_view, 'cp/') !== false) {
				$client_view = str_replace('cp/', 'client/', $_ci_view);
			} else {			
				// otherwise append client to the view
				$client_view = 'client/' . $_ci_view;
			}
			
			$client_path = $this->_load_path($_ci_path, $client_view);
				
			if ( file_exists($client_path) ) {
				// client overide view exists
				$_ci_path = $client_path;
			} else {
				$_ci_path = $this->_load_path($_ci_path, $_ci_view);
			}
